...
/**
 * controller of all components<BR>
 * <BR>
 * BUG GENERATOR!!!
 * 
 * @author Jimmy801
...
public class CalculatorControl {
	 ...
	 * 
	 * @param keyCode - keyCode defined in {@link KeyEvent}
	...
	private void registerKeyEvent() {
		...
		// Ctrl + C -> copy event
		addKeyBind(rootPane, new KeyStroke[] { KeyStroke.getKeyStroke(KeyEvent.VK_C, InputEvent.CTRL_DOWN_MASK) },
				CalUtils.copyStr, copyAction());

		// Ctrl + V -> paste event
		addKeyBind(rootPane, new KeyStroke[] { KeyStroke.getKeyStroke(KeyEvent.VK_V, InputEvent.CTRL_DOWN_MASK) },
				CalUtils.pasteStr, pasteAction());

		// add event
		// Shift + '=' is the sign '+' -> add event
		addKeyBind(rootPane, new KeyStroke[] { KeyStroke.getKeyStroke(KeyEvent.VK_EQUALS, InputEvent.SHIFT_DOWN_MASK),
				getPressKS(KeyEvent.VK_ADD) }, CalUtils.addStr, opAction(CalUtils.addStr));

		// substrate event
		addKeyBind(rootPane, new KeyStroke[] { getPressKS(KeyEvent.VK_SUBTRACT), getPressKS(KeyEvent.VK_MINUS) },
				CalUtils.subStr, opAction(CalUtils.subStr));

		// multiply event
		// Shift + '8' is the sign '*' -> multiply event
		addKeyBind(rootPane, new KeyStroke[] { KeyStroke.getKeyStroke(KeyEvent.VK_8, InputEvent.SHIFT_DOWN_MASK),
				getPressKS(KeyEvent.VK_MULTIPLY) }, CalUtils.mulStr, opAction(CalUtils.mulStr));

		// divide event
		addKeyBind(rootPane, new KeyStroke[] { getPressKS(KeyEvent.VK_DIVIDE), getPressKS(KeyEvent.VK_SLASH) },
				CalUtils.divStr, opAction(CalUtils.divStr));

		// number event
		for (int i = 0; i < 10; ++i) {
			final String numStr = String.valueOf(i);
			addKeyBind(rootPane,
					new KeyStroke[] { KeyStroke.getKeyStroke(numStr), KeyStroke.getKeyStroke("NUMPAD" + i) }, numStr,
					numBtnAction(numStr));
		}

		// dot event
		addKeyBind(rootPane, new KeyStroke[] { getPressKS(KeyEvent.VK_PERIOD), getPressKS(KeyEvent.VK_DECIMAL) },
				CalUtils.dotStr, dotAction());

		// back event
		addKeyBind(rootPane, new KeyStroke[] { getPressKS(KeyEvent.VK_BACK_SPACE) }, CalUtils.backStr, backAction());

		// sign event
		addKeyBind(rootPane, new KeyStroke[] { getPressKS(KeyEvent.VK_F9) }, CalUtils.signStr, signAction());

		// clear event
		addKeyBind(rootPane, new KeyStroke[] { getPressKS(KeyEvent.VK_ESCAPE) }, CalUtils.clearStr, clearAction());

		// CE event
		addKeyBind(rootPane, new KeyStroke[] { getPressKS(KeyEvent.VK_DELETE) }, CalUtils.currentEmptyStr, ceAction());

		// equal event
		addKeyBind(rootPane, new KeyStroke[] { getPressKS(KeyEvent.VK_ENTER), getPressKS(KeyEvent.VK_EQUALS) },
				CalUtils.eqStr, eqAction());
	}
	 ...
	 * 
	 * @param digit - Action string of number
	...
	private ActionListener otherBtnActionListener(String op) {
		if (op.equals(CalUtils.signStr))
			return signActionListener();
		else if (op.equals(CalUtils.dotStr))
			return dotActionListener();
		else if (op.equals(CalUtils.backStr))
			return backActionListener();
		else if (op.equals(CalUtils.eqStr))
			return eqActionListener();
		else if (op.equals(CalUtils.clearStr))
			return clearActionListener();
		else if (op.equals(CalUtils.currentEmptyStr))
			return ceActionListener();
		else if (CalUtils.isOperator(op))
			return opActionListener(op);
		return null;
	}
	...
	private void signEvent() {
		...
		if (CalUtils.isZeroOrEmpty(operand))
			return;
	...
	private void dotEvent() {
		if ((operand.contains(".")))
			return;
		operator = "";
		if (CalUtils.isNullOrEmpty(operand))
			operand = "0";
		operand += ".";
		calView.setExpLblText(operand);
	}
